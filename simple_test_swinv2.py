...
MODEL_PATH = 'models/asl_swinv2_best.pth'
META_PATH = 'models/asl_swinv2_meta.json'

if not os.path.exists(MODEL_PATH):
    print(f"Model not found: {MODEL_PATH}")
    print(f"Train on Colab first: train_model/colab_train_swinv2.ipynb")
    sys.exit(1)
...
model.eval()
print(f"Swin V2 Base loaded ({meta.get('best_val_acc', 0):.2f}% accuracy)")
...
while True:
    ...
    if hand_detected and (x2-x1) > 20 and (y2-y1) > 20:
        ...
        if frame_counter % 5 == 0 and img_cropped.size > 0:
            try:
                ...
                with torch.no_grad():
                    proba = torch.softmax(model(input_tensor), dim=1)[0].numpy()
                mx = np.argmax(proba)
                score = proba[mx] * 100
                res = categories[mx][0]
    ...
    frame_counter += 1
    cv2.imshow("Arabic Sign Language - Swin V2 (99.4%)", img)
